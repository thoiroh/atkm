...
export class AtkShuttleComponent implements OnInit {
  ...
  constructor() {
    this.tools.consoleGroup({ // TAG AtkShuttleComponent -> constructor() ================ CONSOLE LOG IN PROGRESS
      title: `AtkShuttleComponent -> ngOnInit() -> this.configType(): ${this.configType()}`, tag: 'check', palette: 'su', collapsed: true,
    ...
    effect(() => {
      ...
      this.tools.consoleGroup({ // TAG AtkShuttleComponent -> constructor() -> effect() ================ CONSOLE LOG IN PROGRESS
        title: 'AtkShuttleComponent -> Effect: Endpoint/Params Changed',
        tag: 'check',
        palette: 'ac',
        collapsed: true,
  ...
  ngOnInit(): void {
    this.tools.consoleGroup({ // TAG AtkShuttleComponent -> ngOnInit() ================ CONSOLE LOG IN PROGRESS
      title: `AtkShuttleComponent -> ngOnInit() -> this.configType(): ${this.configType()}`, tag: 'check', palette: 'in', collapsed: true,
      data: { configType: this.configType() }
    });
    ...
    if (!config) {
      this.tools.consoleGroup({ // TAG AtkShuttleComponent -> ngOnInit() ERROR: No config found ================ CONSOLE LOG IN PROGRESS
        title: 'AtkShuttleComponent -> ngOnInit() ERROR: No config found', tag: 'cross', palette: 'er', collapsed: true,
        data: { configType: this.configType() }
      });
      return;
    }
    ...
    this.tools.consoleGroup({ // TAG AtkShuttleComponent -> ngOnInit() SUCCESS ================ CONSOLE LOG IN PROGRESS
      title: 'AtkShuttleComponent -> ngOnInit() SUCCESS', tag: 'check', palette: 'ac', collapsed: true,
      data: {
        config: config.id,
    ...
    switch (domain) {
      ...
      default:
        this.tools.consoleGroup({
          title: 'AtkShuttleComponent -> getConfigForDomain() ERROR',
          tag: 'cross',
          palette: 'er',
          collapsed: false,
  ...
  public async loadData(): Promise<void> {
    ...
    if (!endpointConfig) {
      this.tools.consoleGroup({
        title: 'AtkShuttleComponent -> loadData() SKIP: No endpoint config',
        tag: 'check',
        palette: 'wa',
        collapsed: true,
    ...
    this.tools.consoleGroup({
      title: 'AtkShuttleComponent -> loadData() START',
      tag: 'check',
      palette: 'in',
      collapsed: true,
      data: {
    ...
    try {
      ...
      if (response.error) {
        ...
        this.tools.consoleGroup({
          title: 'AtkShuttleComponent -> loadData() ERROR',
          tag: 'cross',
          palette: 'er',
          collapsed: false,
          data: {
      ...
      this.tools.consoleGroup({
        title: 'AtkShuttleComponent -> loadData() SUCCESS',
        tag: 'check',
        palette: 'su',
        collapsed: true,
        data: {
      ...
      this.tools.consoleGroup({
        title: 'AtkShuttleComponent -> loadData() EXCEPTION',
        tag: 'cross',
        palette: 'er',
        collapsed: false,
        data: {
