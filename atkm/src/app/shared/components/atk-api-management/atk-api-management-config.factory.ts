// src/app/shared/components/atk-api-management/atk-api-management-config.factory.ts
// Factory for creating API management configurations with TypeScript objects
...
export class AtkApiManagementConfigFactory {

  /**
   * Create Binance account configuration (v1 - focused on account)
   */
  ...
  /**
   * Create Binance debug configuration (v2 - comprehensive)
   */
  createBinanceDebugConfig(): IBashConfig {
    return {
      id: 'binance-debug-v2',
      title: 'Binance API Enhanced Debug Terminal',
      subtitle: 'Enhanced debugging with ATK API Management architecture',
      defaultEndpoint: 'account',
      terminal: {
        editable: true,
        height: '200px',
        ...
        customCommands: [
          ...
          {
            name: 'refresh',
            description: 'Refresh current endpoint data',
            handler: () => console.log('Refreshing data...')
          },
          {
            name: 'export',
  ...
  createIbkrConfig(): IBashConfig {
    return {
      ...
      endpoints: [
        // Will be implemented later when IBKR integration is ready
        {
          id: 'account',
          name: 'Account Information',
          url: 'http://localhost:8000/ibkr/account',
          method: 'GET',
  ...
  /**
   * Generic configuration creator for new APIs
   */
  createGenericConfig(options: {
    id: string;
    title: string;
    subtitle?: string;
    ...
    return {
      id: options.id,
      title: options.title,
      subtitle: options.subtitle || 'API Debug Terminal',
      defaultEndpoint: options.defaultEndpoint || (options.endpoints[0]?.id || ''),
      terminal: {
        editable: true,
        height: '300px',
        showControls: true,
        customCommands: [
          ...
          },
          {
            ...
            handler: () => console.log('Available commands: clear, help, test')
          },
          {
            name: 'test',
            description: 'Test specific endpoint',
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          type: 'currency',
          sortable: true,
          visible: false // Hidden for now until price conversion is implemented
        }
      ]
    };
  }
    ...
    return {
      ...
      dataTransformer: (apiResponse: any) => {
        ...
        return tickers.map((ticker: any, index: number) => ({
          ...
          // Add mock change data (would come from 24hr ticker in real implementation)
